...
sub sendMail ($self,$cfg) {
    ...
    my $bcfg = $self->app->config->cfgHash->{GENERAL};
    my $from = $cfg->{from} // $bcfg->{mailFrom};
    if ($ENV{OVERRIDE_TO}) {
        $self->log->info("Overriding $cfg->{to} with $ENV{OVERRIDE_TO}");
        $cfg->{to} = $ENV{OVERRIDE_TO};
    }
    my $to = $cfg->{to};
    my @bcc = ($bcfg->{mailBcc} ? ($bcfg->{mailBcc}) : ());
    eval {
    ...
    if ($@) {
        $self->log->warn($@);
        die ["Failed to send mail to $cfg->{to}"];
    }
}
